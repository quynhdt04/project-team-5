var FileUploadWithPreview = function(Q) {
    ...
    class k {
        constructor(A, g = {}) {
            ...
            this.options.showDeleteButtonOnImages = t ?? !0, this.options.maxFileCount = C ?? 0, this.options.presetFiles = I ?? [], this.options.multiple = B ?? !1, this.options.accept = E ?? this.options.accept;
            const {
                browse: s,
                chooseFile: f,
                label: y,
            ...
            this.options.text.chooseFile = f ?? this.options.text.chooseFile, this.options.text.browse = s ?? this.options.text.browse, this.options.text.label = y ?? d, this.options.text.selectedCount = D ?? this.options.text.selectedCount;
    ...
    `;
            ...
            if (u != null && m != null && h != null && J != null) this.inputHidden = u, this.inputVisible = m, this.inputVisible.innerHTML = this.options.text.chooseFile, this.imagePreview = h, this.clearButton = J;
            ...
            this.options.images.baseImage = P ?? this.options.images.baseImage, this.options.images.successPdfImage = L ?? this.options.images.successPdfImage, this.options.images.successVideoImage = G ?? this.options.images.successVideoImage, this.options.images.successFileAltImage = U ?? this.options.images.successFileAltImage, this.options.images.backgroundImage = M ?? this.options.images.backgroundImage, this.addImagesFromPath(this.options.presetFiles), this.addBrowseButton(this.options.text.browse), this.imagePreview.style.backgroundImage = `url("${this.options.images.baseImage}")`, this.bindClickEvents()
        }
        bindClickEvents() {
            ...
            }, !0), this.clearButton.addEventListener("click", () => {
                const A = {
                        detail: {
            ...
            }, !0), this.imagePreview.addEventListener("click", A => {
                const g = A.target;
                if (g) {
        ...
        async addImagesFromPath(A) {
            A.forEach(async g => {
                try {
                    const E = "image/jpeg",
                        B = await (await fetch(g, {
                            mode: "cors"
                        })).blob(),
                        I = new File([B], "preset-file", {
                            type: B.type || E
                        });
                    this.addFiles([I])
                } catch (E) {
                    E instanceof Error && console.warn(`${E.message.toString()}`), console.warn("Image cannot be added to the cachedFileArray.")
                }
            })
        }
        addFiles(A) {
            ...
            this.options.multiple || (this.cachedFileArray = []), g.forEach(B => {
        ...
        addFileToPreviewPanel(A) {
            this.cachedFileArray.length === 0 ? this.inputVisible.innerHTML = this.options.text.chooseFile : this.cachedFileArray.length === 1 ? this.inputVisible.textContent = A.name.split(c)[0] : this.inputVisible.innerHTML = `${this.cachedFileArray.length} ${this.options.text.selectedCount}`;
            const g = new FileReader;
            g.readAsDataURL(A), g.onload = () => {
                if (!this.options.multiple) {
                    let B = this.options.images.successFileAltImage;
                    A.type.match("image/png") || A.type.match("image/jpeg") || A.type.match("image/webp") || A.type.match("image/gif") ? B = `url("${g.result}")` : A.type.match("application/pdf") ? B = `url("${this.options.images.successPdfImage}")` : A.type.match("video/*") && (B = `url("${this.options.images.successVideoImage}")`), this.imagePreview.style.backgroundImage = B;
                    return
                }
      ...
      `
        ...
        refreshPreviewPanel() {
            ...
            Array.from(g).forEach(C => C.classList.add(R)), setTimeout(() => {
    ...
    return Q.DEFAULT_BACKGROUND_IMAGE = l, Q.DEFAULT_BASE_IMAGE = a, Q.DEFAULT_SUCCESS_FILE_ALT_IMAGE = r, Q.DEFAULT_SUCCESS_PDF_IMAGE = o, Q.DEFAULT_SUCCESS_VIDEO_IMAGE = n, Q.Events = e, Q.FileUploadWithPreview = k, Object.defineProperty(Q, Symbol.toStringTag, {
        value: "Module"
    }), Q
}({});
